...
export function buildGovernorReviewV4(manifest = readLatestManifestOrCreate()) {
  const forecast = readJson("data/v4/governor-forecast.json", { races: [] });
  ...
  const review = {
    ...artifactHeader(manifest, "governor-review"),
    sourceForecast: "data/v4/governor-forecast.json",
    rows
  };
  writeJson("data/v4/diagnostics/governor-review.json", review);
  return review;
}

if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replace(/\\/g, "/"))) {
  console.log(`v4 governor review rows: ${buildGovernorReviewV4().rows.length}`);
}
